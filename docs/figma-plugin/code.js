...
async function text(parent, name, value, x, y, w, h, size = 13, color = C.text, weight = 'Regular') {
  const n = figma.createText(); n.name = name; n.x = x; n.y = y;
  n.fontName = { family: 'Inter', style: weight }; n.fontSize = size; n.fills = [solid(color)];
...
function chip(parent, label, x, y, w, fill, color = C.darkText) {
  ...
  return text(parent, `chip label/${label}`, label, x + 12, y + 7, w - 24, 18, 12, color, 'Bold');
}
async function table(parent, spec) {
  ...
  await text(parent, `table/${spec.name}/title`, spec.name, spec.x + 20, spec.y + 15, spec.w - 40, 20, 16, C.darkText, 'Bold');
  if (spec.pin) {
    rect(parent, `table/${spec.name}/pin badge`, spec.x + spec.w - 58, spec.y + 12, 40, 22, C.amberSoft, null, 11);
    await text(parent, `table/${spec.name}/pin text`, 'PIN', spec.x + spec.w - 47, spec.y + 17, 30, 12, 10, '#92400E', 'Bold');
  }
  let y = spec.y + 68;
  ...
  if (spec.note) {
    ...
    await text(parent, `note text/${spec.name}`, spec.note, spec.x + 26, spec.y + spec.h - 30, spec.w - 52, 14, 11, '#3730A3', 'Bold');
  }
}
...
async function main() {
  ...
  await text(root, 'title', 'SchemeOnYou · DB diagram editor mockup', 64, 52, 900, 44, 30, '#F8FAFC', 'Bold');
  ...
  await text(root, 'top/app name', 'SchemeOnYou', 192, 166, 120, 20, 15, C.text, 'Bold');
  ...
  await text(root, 'space help text', 'Space help', 1242, 166, 75, 18, 12, '#EFF6FF', 'Bold');

  await text(root, 'tree title', 'Project tree', 108, 244, 150, 20, 14, C.text, 'Bold');
  await text(root, 'tree diagrams', 'DIAGRAMS', 108, 276, 80, 16, 12, C.muted, 'Bold');
  rect(root, 'active diagram row', 104, 294, 178, 34, C.blue, null, 9);
  await text(root, 'active diagram text', 'DB overview', 118, 304, 120, 18, 13, '#EFF6FF', 'Bold');
  await text(root, 'tree sequence', 'Sequence: checkout', 118, 344, 150, 18, 13, C.slate);
  await text(root, 'tree tables label', 'TABLES', 108, 390, 80, 16, 12, C.muted, 'Bold');
  for (const [i, n] of ['users', 'orders', 'roles', 'user_roles'].entries()) await text(root, `tree/${n}`, n, 118, 416 + i * 28, 120, 18, 13, C.slate);
  rect(root, 'f6 hint', 108, 746, 156, 42, '#0F172A', C.line, 12);
  await text(root, 'f6 hint text', 'F6 next area', 126, 760, 120, 18, 12, C.muted);
  ...
  await text(root, 'canvas label', 'CANVAS', 340, 244, 80, 16, 12, '#64748B', 'Bold');
  ...
  await text(root, 'sheet title', 'A · Add…', 454, 724, 120, 20, 14, C.text, 'Bold');
  ...
  // Inspector
  await text(root, 'inspector title', 'Inspector', 1174, 244, 120, 20, 14, C.text, 'Bold');
  await text(root, 'selected label', 'SELECTED', 1174, 278, 80, 16, 12, C.muted, 'Bold');
  rect(root, 'selected card', 1170, 298, 166, 76, C.panel2, C.line, 14);
  await text(root, 'selected card title', 'orders.user_id', 1186, 316, 140, 18, 13, C.text, 'Bold');
  await text(root, 'selected card subtitle', 'Column · FK source', 1186, 340, 140, 18, 12, C.muted);
  await text(root, 'fk preview label', 'FOREIGN KEY PREVIEW', 1174, 404, 150, 16, 12, C.muted, 'Bold');
  ...
  await text(root, 'quick actions', 'QUICK ACTIONS', 1174, 622, 120, 16, 12, C.muted, 'Bold');
  rect(root, 'create fk button', 1170, 642, 166, 36, C.blue, null, 12);
  await text(root, 'create fk text', 'Create FK', 1192, 653, 90, 16, 12, '#EFF6FF', 'Bold');
  rect(root, 'keep pinned button', 1170, 690, 166, 36, C.panel2, C.line, 12);
  await text(root, 'keep pinned text', 'Keep pinned: off', 1192, 701, 120, 16, 12, C.slate, 'Bold');

  rect(root, 'context line', 108, 854, 1224, 28, C.panel2, null, 10);
  await text(root, 'context label', 'Context:', 126, 862, 64, 16, 13, '#FDE68A', 'Bold');
  ...
  await text(root, 'status value', 'Status: Canvas · Enter edit · Space commands · F1 help · F6 next area', 126, 892, 720, 16, 13, C.muted);
  ...
  await text(shortcutFrame, 'shortcuts title', 'Keyboard shortcuts', 22, 20, 220, 22, 16, C.text, 'Bold');
  const shortcuts = [
    ['Ctrl+Shift+P', 'Command palette'], ['F6 / Shift+F6', 'Next / previous major area'],
  ...
  for (const [i, row] of shortcuts.entries()) {
    const y = 56 + i * 18;
    await text(shortcutFrame, `shortcut/key/${i}`, row[0], 22, y, 112, 16, 11, '#BFDBFE', 'Bold');
  ...
  await text(stateFrame, 'states title', 'Interaction states', 22, 20, 220, 22, 16, C.text, 'Bold');
  const states = [
  ...
  for (const [i, row] of states.entries()) {
    ...
    await text(stateFrame, `state/name/${i}`, row[0], 42, y, 135, 16, 11, C.text, 'Bold');
  ...
  await text(tokenFrame, 'tokens title', 'Design tokens', 22, 20, 220, 22, 16, C.text, 'Bold');
  ...
  for (const [i, token] of tokens.entries()) {
    ...
    await text(tokenFrame, `token label/${token[0]}`, token[0], x + 38, y + 7, 100, 14, 11, C.slate, 'Bold');
  }
