...
// Figma Plugin API source generated from design/final-summary.md and db-diagram-ui-ux-v07.md.
...
async function main() {
  ...
  const root = figma.createFrame(); root.name = 'SchemeOnYou DB diagram editor — v07 mockup';
  place(root, 0, 0, 1440, 1040); root.fills = [solid(C.bg)]; page.appendChild(root);

  await text(root, 'title', 'SchemeOnYou · DB diagram editor mockup', 64, 52, 900, 44, 30, '#F8FAFC', 'Bold');
  await text(root, 'subtitle', 'Keyboard-first JavaFX canvas · generated by Figma Plugin source from final-summary.md', 64, 88, 980, 24, 15, C.slate);
  ...
  // Summary cards as separate frames for designers
  const notes = figma.createFrame(); notes.name = 'Design notes from final-summary.md'; place(notes, 64, 966, 980, 60); notes.fills = [solid(C.panel2)]; notes.cornerRadius = 14; root.appendChild(notes);
  await text(notes, 'notes text', 'Figma source integration: this plugin builds native Figma layers, not a flat screenshot. Key UX: Space command sheet, single context line, source/target FK chips, explicit Keep target pinned.', 20, 20, 930, 20, 13, C.slate);

  figma.viewport.scrollAndZoomIntoView([root]);
  figma.notify('SchemeOnYou DB UI mockup created as native Figma layers');
  figma.closePlugin();
}
