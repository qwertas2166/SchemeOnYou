...
`027-wire-missing-mvp-global-shortcuts.md` is no longer current `work/`; its canonical history stub is `machine/issue/sprint_4/027-wire-missing-mvp-global-shortcuts.md` with `status - done`. Older `work/027` mentions are historical only and must not be used as active scheduler input.

## Scheduler-2 execution — 2026-05-19 19:30 Europe/Moscow

- Взята задача: `025-db-composite-constraints-and-relation-semantics.md` — high/M, ближайшая самая приоритетная незакрытая задача при пустом `backlog/`; уже была `in_progress`, поэтому безопаснее продолжить ее, чем открывать новый широкий work-track.
- Выполнено: финализирован semantic slice — добавлены `DbRelationSemantics`/`DbRelationMeaning` для derived FK meaning (`many-to-one` / `one-to-one` / ambiguous target) и many-to-many join-table detection по двум FK + composite PK/UNQ; validator переведен на общий uniqueness helper.
- Проверки: `mvn -q -pl core -Dtest=DatabaseConstraintValidationTest test`, `mvn -q -pl core test`, `git diff --check` — passed.
- Статус: `025` переведена в `done`; блокеров нет. Широкий незакоммиченный фон проекта сохранен.
