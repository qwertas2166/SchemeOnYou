...
## 2026-05-19 correction — work path wording

Earlier wording that `024` and `033` were in `work/` with `status - done` is historical/stale. Their canonical restored history stubs are under `machine/issue/sprint_9/`; `work/` is not used for done issue files.
