...
progress - 2026-05-17 13:05 MSK scheduler-1: moved from backlog to sprint_13; priority set to high (следующий логичный seam после inspector/edit work; нужен для headless tests).
progress - 2026-05-19 17:00 MSK scheduler-2: backlog was empty, so took the highest-priority eligible planned M/S task: this high/M inspector presenter slice. Verified existing implementation: `InspectorPresenter` provides headless models for diagram/table/column/FK preview/sequence participant/message and edit decisions; `SchemeOnYouApplication` wires it as thin JavaFX layer for inspector fields/commit filtering. Ran targeted Maven check with local Java/Maven: `mvn -q -pl client -am -Dtest=InspectorPresenterTest -Dsurefire.failIfNoSpecifiedTests=false test` OK. Marked done; no production code changes in this pass.
